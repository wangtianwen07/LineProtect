...
public class AppBak {
			...
			checkBox.addItemListener(new ItemListener() {
				...
				public void itemStateChanged(ItemEvent e) {
					...
					if (c instanceof JCheckBox) {
						...
						if(checks.size()>0 && checks.contains("差动保护")) {
							projects.add("相电流差动保护");
							projects.add("零序差动过流保护");
							projects.add("工频变化量阻抗");
	...
	public static void drawCenterContent(JPanel panelCenter, List<String> checks) {
		...
		//一、差动保护
		if(checks.contains("差动保护") || checks.contains("相电流差动保护")) {
			new XdlcdbhView(panelCenter);
		}
		if(checks.contains("差动保护") || checks.contains("零序差动过流保护")) {
			new LxcdglbhView(panelCenter);
		}
		if(checks.contains("差动保护") || checks.contains("工频变化量阻抗")) {
			new GpbhlzkDxView(panelCenter);//单相接地故障
			new GpbhlzkXjView(panelCenter);//相间故障
		}
		//二、纵联距离保护
		if(checks.contains("纵联距离保护") || checks.contains("工频变化量方向保护")) {
			new GpbhlfxbhDxView(panelCenter);//单相接地故障
			new GpbhlfxbhXjView(panelCenter);//相间故障
		}
		if(checks.contains("纵联距离保护") || checks.contains("纵联零序保护")) {
